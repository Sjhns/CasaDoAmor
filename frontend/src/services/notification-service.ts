...
const mapNotificationFromAPI = (apiNotification: NotificationResponse): Notification => {
  return {
    id: apiNotification.id,
    type: apiNotification.tipoAlerta as 'quase-vencido' | 'vencido' | 'quase-esgotado' | 'esgotado',
    medicineName: apiNotification.nomeMedicamento,
    expiryDate: apiNotification.dataVencimento ? new Date(apiNotification.dataVencimento) : undefined,
...
export const notificationService = {
  async getUnread(): Promise<Notification[]> {
    const response = await axios.get<NotificationResponse[]>(`${API_URL}/nao-lidas`);
