import { Search, Filter, ArrowRight, Pencil, AlertTriangle, X } from 'lucide-react';
...
export const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-sky-600 flex">
      {/* Barra Lateral */}
      <aside className="w-64 bg-sky-600 text-white p-6 flex flex-col">
      ...
      <div className="flex-1 flex flex-col">  
        {/* Barra Superior */}
        <header className="bg-sky-600 px-8 py-4 flex items-center justify-between">
          <div className="flex-1 max-w-xl">
            <div className="relative">
              ...
              <button
                className="absolute right-0 top-0 h-full px-3 flex items-center justify-center  rounded-r-lg transition-colors cursor-pointer"
              >
        ...
        <main className="flex-1 bg-gray-50 p-8">
          <div className="bg-white rounded-2xl shadow-sm p-8 h-full">
            <h1 className="text-3xl font-bold text-gray-900 mb-8">Estoque geral de remédios</h1>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-100">
                    ...
                    <th className="text-left px-6 py-4 text-sm font-semibold text-gray-700">Lote</th>
                    ...
                    <th className="text-left px-6 py-4 text-sm font-semibold text-gray-700">Propósito</th>
                ...
                <tbody>
                  {remedios.map((remedio, index) => (
                    ...
                    >
                      <td className="px-6 py-4 font-medium">{remedio.id}</td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3 justify-end">
                          <button className={`p-1.5 hover:bg-opacity-10 rounded transition-colors ${
                            remedio.status === 'vencido' ? 'hover:bg-white' : 'hover:bg-gray-200'
                          }`}>
                            <ArrowRight className={`w-5 h-5 ${
                              remedio.status === 'vencido' ? 'text-white' : 'text-gray-600'
                            }`} />
                          </button>
                          <button className={`p-1.5 hover:bg-opacity-10 rounded transition-colors ${
                            remedio.status === 'vencido' ? 'hover:bg-white' : 'hover:bg-gray-200'
                            ...
                            <Pencil className={`w-5 h-5 ${
                              remedio.status === 'vencido' ? 'text-white' : 'text-green-500'
                            }`} />
                          </button>
                        </div>
                      </td>
